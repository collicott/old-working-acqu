...
//
// This is a function of tagger channel, and NOT bin!
//
Double_t WeightedPar( TFile* file, TString par, UInt_t chan_lo, UInt_t chan_hi,
		UInt_t j = 0)
{
	UInt_t i;
	Double_t quant, val, sum;
	TString scalers;
	...
	for ( i = chan_lo; i <= chan_hi; i++)
	{
		...
		val += hscal->GetBinContent( i+1)*quant;
