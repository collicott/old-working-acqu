		...
		Double_t open_cut = 10)
{
	...
	name = Form( "histograms/MonteCarlo/%s_%d.root", (const char*) reaction,
			eg[ebin]);
	back = new TFile( name);
...
//	name = Form( "E_{#gamma} = %3d #pm 1 MeV", eg[ebin]);
	...
	name = Form( "efficiency/MMissCut%d_%s_%d_%d", (int) open_cut,
			(const char*) reaction, eg[ebin], th[tbin]);
//	name.Append( ".eps");
	name.Append( ".pdf");
//	c1->Print( name);
}

void Plot2D( Int_t ebin = 8, TString proc = "comp", TString tgt = "p")
{
	Double_t left, right;
	...
	name = Form( "plots/pdf/Mmiss_2D_%s_%d", (const char*) reaction, eg[ebin]);
		...
		Double_t open_cut = 10)
{
	...
	name = Form( "histograms/MonteCarlo/%s_%d.root", (const char*) proc,
			eg[ebin]);
	comp = new TFile( name);
...
//	name = Form( "plots/pdf/comp_%s_%d", (const char*) proc, eg[ebin]);
//	name.Append( "_cut");
//	name.Append( ".eps");
//	name.Append( ".pdf");
//	c1->Print( name);
