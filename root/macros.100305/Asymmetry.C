		...
		TString subt = "subt")
{
	...
	name = Form( "plots/AsymBin_%s_%d-%d_t%d.pdf", (const char*) subt, chan_lo,
			chan_hi, tbin);
	c1->Print( name);
}
		...
		TString subt = "subt")
{
	...
	name = Form( "plots/Asymmetry_%s_%d-%d.pdf", (const char*) subt, chan_lo,
			chan_hi);
	c1->Print( name);
}
...
Double_t WeightedEg( UInt_t chan_lo, UInt_t chan_hi)
{
	...
	{
		eg += hscal->GetBinContent( i+1)*tcd[i].energy;
		sum += hscal->GetBinContent( i+1);
	}
	eg /= sum;
...
// for either "full", "empty", or "subt", and everything is
// weighted to Full Para scalers.
void Get2DEmpty( UInt_t chan_lo, UInt_t chan_hi, UInt_t tbin)
{
	...
	hsc_perp_e = (TH1D*) hsc->Clone( "sc_perp_e");
	asym1 = (TH1D*) h2_perp_e->ProjectionX( "perp_e", tb_lo[tbin], tb_hi[tbin]);
	c1->cd( 1);
	asym1->Draw( "e");
	...
	hsc_para_e = (TH1D*) hsc->Clone( "sc_para_e");
	asym2 = (TH1D*) h2_para_e->ProjectionX( "para_e", tb_lo[tbin], tb_hi[tbin]);
	c1->cd( 2);
	asym2->Draw( "e");
	...
	diff2d->Sumw2();
	diff2d->Add( h2_perp_e, -factor);
	asym3 = (TH1D*) diff2d->ProjectionX( "diff1d", tb_lo[tbin], tb_hi[tbin]);
	c1->cd( 3);
	asym3->Draw();
	...
	sum2d->Sumw2();
	sum2d->Add( h2_perp_e, factor);
	asym4 = (TH1D*) sum2d->ProjectionX( "sum1d", tb_lo[tbin], tb_hi[tbin]);
	c1->cd( 4);
	asym4->Draw();
	...
	hA->Sumw2();
	hA->Divide( sum2d);
	asym5 = (TH1D*) hA->ProjectionX( "asym", tb_lo[tbin], tb_hi[tbin]);
	c1->cd( 5);
	asym5->Draw();
}
