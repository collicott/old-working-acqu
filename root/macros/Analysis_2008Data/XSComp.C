...
TFile full( "histograms/Compton/Full/Perp.root");
...
//Int_t tchan[] = { 275, 272, 269, 266, 263, 260, 257, 254, 251, 248, 245};
//Int_t tchan[] = { 286, 
...
void TotXSBin()
{
	...
	// For total cross sections, the theta bin is 0 for 0-180 deg.
	for ( ebin = 0; ebin <= 24; BinXS( ebin++, 0));
	...
	if ( ( scal != 0) && ( tcdbin[ebin].edet > 0)) {
		...
		if ( j == 0) {
			name = Form( "Channel = %d (%d-%d)  Egamma = %5.1f (%5.1f-%5.1f)"
					...
					tcdbin[ebin].etag,  yield, dyield, scal,
