...
{
	UInt_t ebin, tbin, msep, msep1, msep2, msepbest;
	Double_t ratio, ratiobest;
	TString name;

	name = Form( "xs/msep_cuts_Cristina.h");
	...
	name = Form( "       Double_t msep_cut[%2d][10];", nebin);
	cout << name << endl;
	outFile << name << endl;
...
// cout << name << endl;
		...
		if (ebin <=9) {
			if (msepbest == 0) {
			name = Form( "       msep_cut[%1d][%1d] = %1d;      %7.3f", ebin, tbin, msepbest, ratiobest);
			cout << name << endl;

			name = Form( "       msep_cut[%1d][%1d] = %1d;", ebin, tbin, msepbest);
			outFile << name << endl;
			}
			else {
			name = Form( "       msep_cut[%1d][%1d] = %3d;      %7.3f", ebin, tbin, msepbest, ratiobest);
			cout << name << endl;
			name = Form( "       msep_cut[%1d][%1d] = %3d;", ebin, tbin, msepbest);
			outFile << name << endl;
			}
		}
		else {
			if (msepbest == 0) {
			name = Form( "       msep_cut[%2d][%1d] = %1d;      %7.3f", ebin, tbin, msepbest, ratiobest);
			cout << name << endl;

			name = Form( "       msep_cut[%2d][%1d] = %1d;", ebin, tbin, msepbest);
			outFile << name << endl;
			}
			else {
			name = Form( "       msep_cut[%2d][%1d] = %3d;      %7.3f", ebin, tbin, msepbest, ratiobest);
			cout << name << endl;
			name = Form( "       msep_cut[%2d][%1d] = %3d;", ebin, tbin, msepbest);
			outFile << name << endl;
			}
		}
...
void singlechannel( UInt_t ebin = 0, UInt_t tbin = 9)
{
	UInt_t msep;
	Double_t open_cut;
	TString name;
	...
	name = Form( "Energy = %3d MeV, Theta = %3d, Bin[%3d][%3d]", eg[ebin], th[tbin], ebin, tbin);
	cout << name << endl;

		for ( msep = 1000; msep <= 1050; msep++) {
			CompEffBin( ebin, tbin, msep, open_cut, "pi0_p");

			name = Form( "%3d %6.4f %6.9f", msep, effp, cont);
			cout << name << endl;
		}
