...
Int_t eg[] = { 289, 291, 294, 296, 298, 301, 303};
...
Int_t tchan[] = {249, 248, 247, 246, 245, 244, 243};

Double_t effp, cont, contthresh;
contthresh = 0.1;
Int_t nebin = 7;    //Number of array elements in eg[],  ex: eg[] = {240,243,245} , nebin = 3
...
{
	UInt_t ebin, ebinout, tbin, msep, msep1, msep2, msepbest, whichmsep;
	Double_t ratio, ratiobest, contout;
	TString name;

	name = Form( "xs/msep_cuts_3.h");
...
//	cout << name << endl;
		...
		if (msepbest == 0) {
		name = Form( "       msep_cut[%2d][%1d] = %1d;      %7.3f , %d", ebinout, tbin, msepbest, contout, whichmsep);
		cout << name << endl;
		...
		name = Form( "       msep_cut[%2d][%1d] = %3d;      %7.3f , %d", ebinout, tbin, msepbest, contout, whichmsep);
		cout << name << endl;
