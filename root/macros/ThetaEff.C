...
void ThetaEff( UInt_t chan = 300, TString experiment = "pi0",
		TString encl = "LH2_10cm", TString proc = "pi0_p", Bool_t plot = kTRUE,
		UInt_t rebin = 20)
{
	Int_t i, end;
	Int_t yld_ev, yld_ge;
	...
	base = Form( "%s_chan%d", (const char*) proc, chan);

	name = "evgen/" + base + "_in.root";
	evgen = new TFile( name);

	name = "histograms/MonteCarlo/" + experiment + "/" + encl + "/" + base +
		".root";
...
//	name = "THR_ThetaCMP";
	...
	name = Form( "xs/pi0/DetEff_chan_%d.out", chan);
...
void ThetaEffEff( Bool_t plot = kTRUE, TString tgt = "p", Int_t eg = 100)
{
	...
	base = Form( "eff_%s_%d", (const char*) tgt, eg);
	...
	name = Form( "xs/eff/det_eff_new/Eff_%s_%d.out", (const char*) tgt, eg);
	ofstream outFile( name);
