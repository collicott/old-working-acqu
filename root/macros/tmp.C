...
TFile full( "histograms/FullCompton.root");
...
Bool_t BinXS( UInt_t ebin, UInt_t theta_bin) 
{
	...
	tageff = WeightedPar( file, "tageff", chan_lo, chan_hi);
	deteff = WeightedPar( file, "deteff", chan_lo, chan_hi, j);
	...
	if ( ( scal != 0) && ( deteff > 0)) {
		...
		tcd[i].xs[j] = n_yld*fact/deteff/tageff/tbin[j].dom;
		tcd[i].dxs[j] = dn_yld*fact/deteff/tageff/tbin[j].dom;

		if ( j == 0) {
			name = Form( "Channel = %d (%d-%d)  Egamma = %5.1f (%5.1f-%5.1f)"
					"  edet = %4.2f  etag = %4.2f", i, chan_lo, chan_hi, eg, temin,
					temax, deteff, tageff);
			cout << name << endl;
			...
			name = Form( "Channel = %d (%d-%d)  Egamma = %5.1f (%5.1f-%5.1f)", i,
					chan_lo, chan_hi, eg, temin, temax);
			cout << name << endl;
			name = Form( "  CM Theta = %d (%d-%d)  edet = %4.2f  etag = %4.2f",
					tbin[j].th, (int) theta_lo, (int) theta_hi, deteff, tageff);
			cout << name << endl;
			name = Form( "  dom = %5.3f  Y = %6.1f +/- %4.1f  S = %6.2e"
...
void DiffXSChan( UInt_t ebin)
{
	UInt_t j, k, chan_lo, chan_hi;
	Double_t eg, temin, temax;
	...
	for (  j = 1; j <= 9; j++) {
		...
		{
			name = Form( "%3d  %6.4f  %5.4f", tbin[j].th, tcd[ebin].xs[j],
				tcd[ebin].dxs[j]);
			outFile << name << endl;
			tth[k] = tbin[j].theta;
			dtth[k] = 0;
			xxs[k] = tcd[ebin].xs[j];
			dxxs[k++] = tcd[ebin].dxs[j];
		}
	}
	outFile.close();
	...
	// Plot Results
	gr = new TGraphErrors( k, tth, xxs, dtth, dxxs);
	name = Form( "#gammap#rightarrow#gammap Differential Cross Section"
			" E_{#gamma} = %5.1f (%5.1f - %5.1f) MeV", eg, temin, temax);
	gr->SetTitle( name);
	gr->SetMarkerColor( 4);
...
{
	UInt_t i, j, k;
	Double_t eg, tth[9], dtth[9], xxs[9], dxxs[9];
	TString name;
	...
	// Output results to a file
	name = Form( "xs/compton/xs/diff_xs_%d.out", (int) (eg+0.5));
	ofstream outFile( name);
	if ( !outFile.is_open()) {
		cout << "Error opening file ";
		cout << name;
...
*/
	...
	name = Form( "plots/diff_xs_compton_%s_%d", (const char*) subt_str, eg);
//	name.Append( ".eps");
	name.Append( ".pdf");
	c1->Print( name);
}

void DiffComp( UInt_t eg)
{
	Int_t i, ct;
	...
	pt = new TLegend( 0.6, 0.65, 0.8, 0.80);
	pt->SetTextSize( 0.04);
	pt->SetFillColor( 0);
	pt->SetBorderSize( 0);
