...
//--Update      C Collicott...5th Aug 2010...Trigger Added
//--Description
//                *** Acqu++ <-> Root ***
...
enum { ECBAngleLimits = 1000, ECBParticleCuts , ECBTrigThresh };
static const Map_t kCBKeys[] = {
  {"AngleLimits:",    ECBAngleLimits},
  {"ParticleID-Cut:", ECBParticleCuts},
  {"Trigger-Section-Threshold:",   ECBTrigThresh},
...
TA2CrystalBall::~TA2CrystalBall()
{
  ...
  if( fETrigSect ) delete fETrigSect;
  if( fCBThresholds ) delete fCBThresholds;
  if( fTrigFile ) fTrigFile->Close();
}
...
//  TA2DataManager::LoadVariable("DeltaTheta",fDeltaTheta,     EDMultiX);
  ...
  TA2DataManager::LoadVariable("NTrigSect", &fNTrigSectFired,EISingleX);
  TA2DataManager::LoadVariable("ETrigSect", fETrigSect,      EDMultiX);
  TA2DataManager::LoadVariable("ETrigCBA",  &fETrigCBA,      EISingleX);
  TA2DataManager::LoadVariable("ETrigCBB",  &fETrigCBB,      EISingleX);
  TA2DataManager::LoadVariable("ETrigCBC",  &fETrigCBC,      EISingleX);
  TA2DataManager::LoadVariable("ETrigCBD",  &fETrigCBD,      EISingleX);
  TA2DataManager::LoadVariable("Simple",    &fSimple,        EISingleX);
  //
...
void TA2CrystalBall::SetConfig(Char_t* line, Int_t key)
{
  ...
  case ECBAngleLimits:
    ...
    break;
  case ECBTrigThresh:
    //  Trigger Section Threshold
    if( sscanf(line,"%s %s", fTrigFileName, fTrigHistName ) == 2 ){
      fUseTrigHist=kTRUE;
    }
    else if( sscanf(line,"%lf", &fTrigThresh) == 1 ){
      fUseSingleThresh=kTRUE;
    }
    else{
      PrintError(line,"<Trigger Section Threshold declaration>");
      return;
    }
    break;
  default:
...
void TA2CrystalBall::PostInit( )
{
  ...
  fETrigCBD = 0;  
  fSimple = 0;
  fETrigSect = new Double_t[46];
  for(int i=0; i<46; i++){
    fETrigSect[i] = (Double_t)EBufferEnd;
    }
  if(fUseTrigHist){
    TDirectory *olddir = gDirectory;
    fTrigFile=new TFile(fTrigFileName);
    fCBThresholds=(TH1D*)fTrigFile->Get(fTrigHistName);
    olddir->cd();
    if(fCBThresholds){
    }
    else {
...
void TA2CrystalBall::Reconstruct( )
{
  ...
  if(fUseSingleThresh || fUseTrigHist) TriggerSections();
  ...
  if( !nPID ){
    for( Int_t i=0; i<nNaI; i++ ){
      ...
      fParticleID->SetP4(fP4+i, kGamma, energy,
  ...
  for( i=0; i<nNaI; i++ ){                       // loop cluster hits
    ...
    for( j=0; j<nPID; j++ ){
      *diffPID++ = TMath::Abs( fPID->GetPosition( fPID->GetHits(j) )->Z()
			       - cl->GetPhi() );
  ...
  for( i=0; i<nPID; i++ ){
    ...
    if( fPIDindex[j] == ECBNullPartner ) fPIDindex[j] = k;
  ...
  for( i=0,l=0; i<nNaI; i++ ){
    ...
    fParticle[i].SetApparatus(EAppCB);            // Register particvle as CB particle
    ...
    if( (j = fPIDindex[i]) == ECBNullPartner ) fPDG_ID[i] = kGamma;
    else{
	...
	* TMath::Abs( TMath::Sin( cl->GetTheta() * TMath::DegToRad() ) );
      ...
      if(fPID->IsTime())fParticle[i].SetVetoTime(fPID->GetTime(k));
	...
	fDeltaE[k] += fDeltaEdouble[k];
      ...
      if( fMWPC){
	if( (j = fMWPCindex[i]) != ECBNullPartner ){
...
void TA2CrystalBall::TriggerSections()
{
  // Function to calculate the number of trigger sections fired. C.Tarbert 22.11.06

  UInt_t nhits = fNaI->GetNhits();
  Int_t index = -1;                  // which trig section i.e. 0 - 89
  Int_t *hits = fNaI->GetHits();
  const Int_t nSect = 45;              // no of trig sections i.e. 720/8
  Double_t TrigSectE[nSect] = {0.};
  Int_t    TrigSectF[nSect] = {0};
  Double_t *energy = fNaI->GetEnergy();
  Double_t threshold;

  fNTrigSectFired = 0;

  // Loop over all hits in the NaI and sum the energies of crystals fired
  // in the same trigger section.
  for(UInt_t i=0; i<nhits; i++){ 
    // which trig section does this crystal hit belong to...
    index = (int)(hits[i]/16.);            
    // add the energy in that crystal to the correct trigger section...      
    TrigSectE[index] += energy[hits[i]];
    if(fUseSingleThresh) threshold = fTrigThresh;
    //    cout<<"hits[i]+1 = "<<(hits[i]+1)<<endl;
    if(fUseTrigHist && fCBThresholds) {
      threshold = fCBThresholds->GetBinContent((hits[i]+1));
    }
    if (energy[hits[i]]>threshold) TrigSectF[index]= 1;
    //    cout<<"Threshold is "<<threshold<<endl;
  }

  // Sort energies of trigger sections - highest first
  Int_t indices[45] = {0};
  TMath::Sort(45,TrigSectE,indices,true);

  // If the total energy deposited in any one trigger section is greater
  // than trigger threshold read in from CB.dat, increment nTrigSectFired.
  ...
  // Fill buffer with energies of all fired trigger sections
  fETrigSect[0] = (Double_t)EBufferEnd;
  Int_t count = 0;
  for( int i=0; i<fNTrigSectFired; i++ ){
    fETrigSect[count] = TrigSectE[indices[i]];
    count++;
  }
  fETrigSect[count] = EBufferEnd;

  fETrigCBA = fETrigSect[0];
  fETrigCBB = fETrigSect[1];
  fETrigCBC = fETrigSect[2];
  fETrigCBD = fETrigSect[3];
