...
TA2CentralApparatus::TA2CentralApparatus(const char* name, TA2System* analysis) 
		    :TA2Apparatus(name, analysis, kValidDetectors)
{
  ...
  // Zero PIDproton and PIDpion counters //***
//  Int_t fNPIDProton;
//  Int_t fNPIDPion;
//  Int_t fNPIDRootino;
...
void TA2CentralApparatus::LoadVariable( )
{
  ...
  TA2DataManager::LoadVariable("PIDHitsProton", fPIDHitsProton, EIMultiX);
  TA2DataManager::LoadVariable("PIDHitsPion",   fPIDHitsPion,   EIMultiX);
  TA2DataManager::LoadVariable("PIDHitsRootino",fPIDHitsRootino,EIMultiX);
...
void TA2CentralApparatus::PostInit()
{
  ...
  {
    // Particle ID
    fPIDHitsProton  = new Int_t[fMaxParticle];     //Cristina //***
    fPIDHitsPion    = new Int_t[fMaxParticle];     //Cristina //***
    fPIDHitsRootino = new Int_t[fMaxParticle];     //Cristina //***
...
//  {
	...
	break;							// cut OK so exit loop
	  ...
	  if (fType[fNparticle] == 2212) {			//***
	  fPIDHitsProton[fNPIDProton] = iPid;
	  fNPIDProton++;
	  }

	  else if (fType[fNparticle] == 211) {			//***
//	  fPIDHitsPion[fNPIDPion] = track.GetIhitPid();	
	  fPIDHitsPion[fNPIDPion] = iPid;			
	  fNPIDPion++;
	  }

	  else if (fType[fNparticle] == 0) {			//***
	  fPIDHitsRootino[fNPIDRootino] = iPid;		
	  fNPIDRootino++;
	  }
  }
  ...
  //Terminate arrays
  fPIDHitsProton[fNPIDProton]   = EBufferEnd; //***
  fPIDHitsPion[fNPIDPion]       = EBufferEnd; //***
  fPIDHitsRootino[fNPIDRootino] = EBufferEnd; //***
