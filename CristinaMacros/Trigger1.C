...
void TriggeredSpectra(Int_t TriggerCompare = 2, Char_t* process) {
	...
	TH1D *a0p = new TH1D("PhotThetaP", "PhotTheta (Prot)", 180, 0, 180);
	TH1D *a1  = new TH1D("PhotThetaT", "PhotTheta (Trig)", 180, 0, 180);

	TH1D *b0  = new TH1D("PhotPhi",  "PhotPhi",        72, -180, 180);
	TH1D *b0p = new TH1D("PhotPhiP", "PhotPhi (Prot)", 72, -180, 180);
	...
	TH1D *g0  = new TH1D("NPhoton",  "NPhoton", 	   10, 0, 10);
	TH1D *g1  = new TH1D("NPhotonT", "NPhoton (Trig)", 10, 0, 10);

	TH1D *h0  = new TH1D("NProton",  "NProton", 	   10, 0, 10);
	TH1D *h1  = new TH1D("NProtonT", "NProton (Trig)", 10, 0, 10);

	TH2F *gh0 = new TH2F("NPhotNProt", "NPhotNProt",       10,0,10,10,0,10);
	TH2F *gh1 = new TH2F("NPhotNProtTrig","NPhotNProt (Trig)",10,0,10,10,0,10);
	...
	for (Int_t i=0;i<nentries;i++) {
		...
		for ( Int_t a = 0; a< NPhoton; a++) {
			...
			b0->Fill(PhotonPhi[a]);
			if (process == "Compton" && NProton > 0){
				a0p->Fill(PhotonTheta[a]);
				b0p->Fill(PhotonPhi[a]);
			}
		}
		...
		g0->Fill(NPhoton);
		h0->Fill(NProton);
		gh0->Fill(NPhoton,NProton);

		for (a = 0; a< NPattern; a++) {
			if (TriggerPattern[a] == TriggerCompare && NCluster == 2){
				g1->Fill(NPhoton);
				h1->Fill(NProton);
				gh1->Fill(NPhoton,NProton);
        ...
        Char_t* canv = Form("DetectedParticles_%s",process);
	...
	if (process == "Compton"){
		a0p->SetLineColor(kCyan+2);
		a0p->Draw("same");
	}

	a1->Draw("same");

//        canv = Form("PhotPhi_%s",process);
//	canvas2 = new TCanvas(canv,canv);
	...
	if (process == "Compton"){
		b0p->SetLineColor(kCyan+2);
		b0p->Draw("same");
	}

	b1->Draw("same");

//        canv = Form("ProtTheta_%s",process);
...
//        canv = Form("PhotPhiTrig_%s",process);
//	canvas6 = new TCanvas(canv,canv);
	canvas5->cd(2);	
	...
	f2->Draw("same");
        ...
        canv = Form("NPhoton_%s",process);
	canvas7 = new TCanvas(canv,canv);
	canvas7->Divide(2,1);
	canvas7->cd(1);
...
//        canv = Form("NProton_%s",process);
//	canvas8 = new TCanvas(canv,canv);
	canvas7->cd(2);
