...
void TriggeredSpectraM2(Int_t TriggerCompare, Char_t* process, Int_t Energy) {
	...
	Int_t	 NaIClusters[100],BaF2Clusters[10], NaINCluster, BaF2NCluster;
	...
	tree1->SetBranchAddress("NaIClusters",		&NaIClusters);
	tree1->SetBranchAddress("BaF2Clusters",		&BaF2Clusters);
	...
	TH2F *h2DNaICluster  = new TH2F("NaIClusters",	"NaIClusters",	45, 0, 45, 45, 0, 45);
	TH2F *h2DBaF2Cluster = new TH2F("BaF2Clusters",	"BaF2Clusters",	8, 0, 8, 8, 0, 8);
	...
	for (Int_t i=0;i<nentries;i++) {
		...
		for ( a = 0; a < NaINCluster; a++) {
			for ( b = (a+1); b < NaINCluster; b++) {
				
			h2DNaICluster->Fill(NaIClusters[a],NaIClusters[b]);

			}
		}

		for ( a = 0; a < BaF2NCluster; a++) {
			for ( b = (a+1); b < BaF2NCluster; b++) {
				
			h2DBaF2Cluster->Fill(BaF2Clusters[a],BaF2Clusters[b]);
	...
	Char_t* canv = Form("Cluster Pairs-%s",process);
	canvasClus = new TCanvas(canv,canv);
	canvasClus->Divide(2,2);
