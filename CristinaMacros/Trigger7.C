...
void TriggerComparison(Int_t TriggerCompare, Char_t* Mult, Int_t Energy) {
	...
	Int_t MultiplicityOriginal, MultiplicityNeighbour, Multiplicity;
	...
	for (Int_t i=0;i<nentries;i++) {
		...
		if 	(Mult == "old") Multiplicity = MultiplicityOriginal;
		else if (Mult == "new") Multiplicity = MultiplicityNeighbour;
		else	printf("ERROR: Unknown Multiplicity Selection!\n");
	...
	Double_t CompAcc = 100*( a4b->GetBinContent(4) ) / ( a0b->GetBinContent(1) );
	Double_t PionAcc = 100*(  a4->GetBinContent(9) ) / (  a0->GetBinContent(6) );

	printf("Compton Acceptance (adjusted, gp): %4.2f %\n", CompAcc);
	printf("Pi0 Acceptance: %4.2f %\n", PionAcc);

///	TriggeredSpectra(TriggerCompare,Mult,"Compton", Energy);
//	TriggeredSpectra(TriggerCompare,Mult,"Pi0",Energy);

}


void TriggeredSpectra(Int_t TriggerCompare, Char_t* Mult, Char_t* process, Int_t Energy) {
	...
	Int_t	 NaIHits[720],BaF2Hits[384], NaINHits, BaF2NHits;
	Int_t 	 MultiplicityOriginal, MultiplicityNeighbour, Multiplicity;
	Int_t 	 TriggerPattern[20], ComparisonTriggerPattern[10];
	Bool_t   Triggered;
	...
	tree1->SetBranchAddress("NaINHits",		&NaINHits);
	tree1->SetBranchAddress("BaF2NHits",		&BaF2NHits);

	tree1->SetBranchAddress("NaIHits",		&NaIHits);
	tree1->SetBranchAddress("BaF2Hits",		&BaF2Hits);
	...
	TH1D *hhNaIHits      = new TH1D("NaIHits",	"NaIHits",	720, 0, 720);
	TH1D *hhNaIHitsTrig  = new TH1D("NaIHitsTrig",	"NaIHitsTrig",	720, 0, 720);
	TH1D *hhNaIHitsTWM   = new TH1D("NaIHitsTWM",	"NaIHitsTWM",	720, 0, 720);

	TH1D *hhBaF2Hits     = new TH1D("BaF2Hits",	"BaF2Hits",	384, 0, 384);
	TH1D *hhBaF2HitsTrig = new TH1D("BaF2HitsTrig",	"BaF2HitsTrig",	384, 0, 384);
	TH1D *hhBaF2HitsTWM  = new TH1D("BaF2HitsTWM",	"BaF2HitsTWM",	384, 0, 384);
	...
	for (Int_t i=0;i<nentries;i++) {
		...
		if (process == "Compton") {	
		for ( a = 0; a < NaINHits; a++) {
		
			hhNaIHits->Fill(NaIHits[a]);
			hhBaF2Hits->Fill(BaF2Hits[a]);

			if (Triggered == kTRUE){
				hhNaIHitsTrig->Fill(NaIHits[a]);
				hhBaF2HitsTrig->Fill(BaF2Hits[a]);
			}

			if (TriggeredWM == kTRUE){
				hhNaIHitsTWM->Fill(NaIHits[a]);
				hhBaF2HitsTWM->Fill(BaF2Hits[a]);
			}
	...
	if (process == "Compton") {
	        ...
	        Char_t* canv = Form("Hits-%s",process);
		canvas0 = new TCanvas(canv,canv);
		canvas0->Divide(2,1);
		canvas0->cd(1);

		hhNaIHits->SetFillColor(kBlue);
		hhNaIHitsTWM->SetFillColor(kRed);
		hhNaIHitsTrig->SetFillColor(kMagenta+2);
	
		hhNaIHits->Draw();
		hhNaIHitsTWM->Draw("same");
		hhNaIHitsTrig->Draw("same");

		canvas0->cd(2);

		hhBaF2Hits->SetFillColor(kBlue);
		hhBaF2HitsTWM->SetFillColor(kRed);
		hhBaF2HitsTrig->SetFillColor(kMagenta+2);

		hhBaF2Hits->Draw();
		hhBaF2HitsTWM->Draw("same");
		hhBaF2HitsTrig->Draw("same");

		Char_t* canv = Form("Triggered Hits Ratio");
		canvasRatio = new TCanvas(canv,canv);
		TH1D *ratio  = new TH1D("Triggered Hits Ratio","Triggered Hits Ratio",720, 0, 720);
		ratio->Divide(hhNaIHitsTWM,hhNaIHitsTrig,1,1,"");
		ratio->Draw();
	}
